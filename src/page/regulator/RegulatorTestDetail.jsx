...
import "./style/RegulatorTestDetail.css";

const RegulatorTestDetail = () => {
  ...
  const fetchQuestions = async () => {
    try {
      setLoading(true);
      const response = await api.get(`/regulator/test-questions/${chapterId}`);
  ...
  const submitTest = async (finalScore) => {
    try {
      ...
      const response = await api.post(`/regulator/test-submit`, payload);
      if (response.data && response.data.success) {
        // นำทางไปหน้าสรุปผล (RegulatorResult) เพื่อโชว์เซอติฟิเคท
        navigate("/regulator-result", {
          state: {
...
export default RegulatorTestDetail;
